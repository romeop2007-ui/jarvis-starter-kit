...
**Routine de Roméo :** il remplit le DAILY REPORT **le lendemain matin pour la veille**. Quand il demande l'analyse d'hier, il veut des chiffres bruts prêts à copier-coller, pas une analyse commentée. Testing dans `TESTINGS`, scaling dans `DAILY REPORT`, le résultat net d'un testing terminé étant reporté en charge négative dans les Other charges du mois. **Demande type (validée le 13/09/2026) : "nombre de commandes PayPal hier et COGS d'hier"**, avec le COGS de toutes les commandes (dons compris), lu dans le calculateur COGS du Sheet ROAS BE & TARGET (`references/pnl-officiel-formation.md` section 6).
...
**🧭 Ordre des phases à respecter avant toute production de créa (grille officielle de la leçon 1.6, transcrite le 11/09/2026, détail dans `context/CONTEXT.md`) :** phase 1 dès 100 €/jour = **traduire**, 3 à 7 créas par jour jusqu'à 30-40 traduites ; phase 2 = marketing research puis ses propres créas, 50/50 avec la traduction ; phase 3 à partir de 1-2 k/jour de CA = itérations + **70 à 80 statiques** ; phase 4 à 7-8 k/jour = achat de contenu. **Toujours vérifier dans quelle phase se trouve le produit avant de proposer un type de créa** : une statique proposée en phase 1 fait sauter deux phases. **Production de statiques (méthode du 11/09/2026)** : Roméo pilote son GPT custom et Nano Banana Pro ; Claude fournit les requêtes MODE 1 (copie de structure) et les références réelles du Google Sheet « Tops statics » filtré par niche et format, jamais des compositions inventées.
