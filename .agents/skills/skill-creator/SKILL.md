...
| Needs Codex's judgment? | Instructions only — no script, no tool |
...
```bash
python3 .Codex/skills/skill-creator/scripts/init_skill.py SKILL_NAME
```
...
model: sonnet
context: fork
allowed-tools: Bash(python3 .Codex/skills/SKILL_NAME/scripts/*)
---
```
...
### Step 1: Name of Step
\`\`\`bash
python3 .Codex/skills/SKILL_NAME/scripts/script_name.py --flag value
\`\`\`
...
1. **Trigger test:** Fresh session. Ask naturally without naming the skill. Does Codex find it?
...
- Tighten instructions where Codex improvised incorrectly
...
- Runtime behavior settings → `config/`
