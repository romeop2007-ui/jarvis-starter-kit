#!/usr/bin/env python3
"""
Script: mem0 Sync to MEMORY.md (DEPRECATED)
============================================

DEPRECATED as of 2026-05-11. Do not run this under normal circumstances.

Since the native auto-memory migration, layer 1 of the AIOS memory stack lives
at `~/.claude/projects/-root-aios/memory/` and is managed by Claude Code
itself (claude writes to topic files like user.md, system.md, decisions.md as
it learns). The legacy `/root/aios/memory/MEMORY.md` is now a pointer file.

Running this script would write content into the legacy pointer location and
serves no purpose. It is kept on disk only for one-shot manual exports of the
Pinecone state if explicitly needed (for example, a backup snapshot).

See `.claude/rules/memory-protocol.md` for the current architecture.

Original purpose
----------------
Read all mem0 memories, classify into sections, regenerate memory/MEMORY.md
as a human-readable snapshot.

Usage (manual export only)
--------------------------
...
import sys
print(
    "[mem0_sync_md] DEPRECATED. Layer 1 is now the native auto-memory at "
    "~/.claude/projects/-root-aios/memory/, claude-managed. "
    "See .claude/rules/memory-protocol.md. "
    "Add --force to run anyway (one-shot manual export).",
    file=sys.stderr,
)
...
def _list_from_history(limit=200):
    ...
    Gets the latest event per memory_id. Excludes memories whose latest
    event is DELETE (properly handles m.delete() calls).
    """
