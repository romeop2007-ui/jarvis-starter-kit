...
natifs de Shrine Pro, en suivant le SOP de la formation. Pas Claude.

**Claude n'intervient que quand Roméo le demande explicitement**, et seulement sur le
morceau demandé : un élément du concurrent qu'aucun bloc natif ne sait reproduire.

### Pourquoi cette règle existe

Le 31/08/2026, Claude a reconstruit toute la fiche Émeraude Dorée en Liquid (buy box,
...
la note et les accordéons, que Shrine faisait nativement**, parce que Claude copiait la
...
gratuitement, et il crée une dépendance à Claude pour la moindre retouche.
...
justement parce que Roméo ne sollicite Claude que quand le natif ne suffit pas.
...
Le but : si Roméo change sa palette, tout suit sans repasser par Claude.
...
**Ce que Claude ne reproduit pas, même sur demande** : les cautions attribuées à des
...
fictifs, que Roméo a explicitement choisi d'assumer. Claude pose un placeholder neutre aux
...
Par défaut, Claude **livre le bloc dans le chat** et Roméo le colle lui-même. Le push
...
2. **Valider avant de pousser**, non négociable :
   ```bash
   node .claude/skills/fiche-produit/scripts/validate-template.mjs <template>
   ```
...
configuration** : Claude conseille les réglages à reproduire d'après le concurrent et
...
- Pas d'upload de fichier local vers Shopify côté Claude : Roméo dépose les visuels dans
  Admin > Contenu > Fichiers, Claude résout l'URL CDN ensuite.
