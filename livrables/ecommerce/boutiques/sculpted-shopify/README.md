# Sculpted — Boutique Shopify (sections éditables)
...
sculpted-shopify/
├── assets/
│   └── sculpted.css                       # TOUT le CSS (rendu identique à la maquette). Chargé par le layout.
├── layout/
│   └── sculpted.liquid                    # Layout isolé (pas de header/footer Dawn). Rend le chrome partagé.
├── snippets/
...
│   ├── sculpted-press.liquid              # Barre "As seen in" (1 bloc par logo)
│   ├── sculpted-benefits.liquid           # 4 bénéfices autour d'une image
│   ├── sculpted-social-proof.liquid       # Témoignages (1 bloc par carte)
│   ├── sculpted-product.liquid            # Buy box : galerie, puces, paliers de prix, options, UGC, accordéons
│   ├── sculpted-how.liquid                # "How it works" (4 notes + diagramme)
│   ├── sculpted-stats.liquid              # Stats de transformation + avant/après
│   ├── sculpted-comparison.liquid         # Tableau comparatif (1 bloc par ligne)
│   ├── sculpted-size-guide.liquid         # Guide des tailles (1 bloc par taille)
│   ├── sculpted-faq.liquid                # FAQ (1 bloc par question)
│   ├── sculpted-reviews.liquid            # Mur d'avis (1 bloc par avis)
│   ├── sculpted-trust-badges.liquid       # Rangée des 4 garanties (réutilisée partout)
│   │  # --- Contenu page Contact / Track order ---
│   ├── sculpted-contact.liquid            # Intro + formulaire contact natif Shopify
│   └── sculpted-track.liquid              # Bloc de suivi de commande
└── templates/
    ├── index.liquid                     # Homepage : assemble les sections de contenu
    ├── page.contact.json                # Page Contact : contact + badges
    └── page.sculpted-track-order.liquid   # Page Track order : suivi + badges
```

## Comment ça marche (le principe à retenir)

- **Le chrome (ticker + menu + footer)** est rendu par le **layout** `sculpted.liquid`.
...
- **Tout le CSS** vit dans `assets/sculpted.css`. Ne le duplique pas dans les sections :
...
2. **assets/** : Ajouter un asset > `sculpted.css` > colle le contenu du fichier
3. **snippets/** : Ajouter > `sculpted-benefit-icon` > colle le contenu
4. **layout/** : ouvre (ou crée) `sculpted` > colle le contenu de `layout/sculpted.liquid`
...
6. **templates/** :
   ...
   - modèle de **page** nommé `sculpted-track-order` (Liquid) > colle `templates/page.sculpted-track-order.liquid`
   - modèle de **page** nommé `contact` (JSON) > colle `templates/page.contact.json`
7. Assigne les modèles : **Pages** > ta page Contact > Modèle `contact` ; ta page de suivi > Modèle `sculpted-track-order`
...
- La police Archivo est chargée par le layout. La section `sculpted-contact` la recharge
  pour elle-même car la page Contact utilise le thème Dawn (template JSON), pas le layout `sculpted`.
