---
name: memory
description: Query and manage the AIOS long-term memory (mem0 + Pinecone) and the daily logs. Layer 1 is owned by Codex natively.
---

# Memory Skill

Operates on layers 2 and 3 of the AIOS memory stack. Layer 1 (native auto-memory) is managed by Codex itself; see `.Codex/rules/memory-protocol.md` for the full architecture.
...
```bash
python3 .Codex/skills/memory/scripts/smart_search.py --query "topic" --limit 5
```
...
```bash
python3 .Codex/skills/memory/scripts/mem0_search.py --query "topic" --limit 10
```

### Rebuild keyword index
```bash
python3 .Codex/skills/memory/scripts/smart_search.py --rebuild-index
```
Repopulates FTS5 from the history DB. Run after installation or to fix drift.

### Add a fact manually
```bash
python3 .Codex/skills/memory/scripts/mem0_add.py --content "User prefers Pinecone over Supabase for vectors"
```

### Add from conversation messages
```bash
python3 .Codex/skills/memory/scripts/mem0_add.py --messages '[{"role":"user","content":"I switched to ClickUp"}]'
```

### List all memories
```bash
python3 .Codex/skills/memory/scripts/mem0_list.py --limit 50
```

### Delete a memory
```bash
python3 .Codex/skills/memory/scripts/mem0_delete.py --memory-id "abc123"
python3 .Codex/skills/memory/scripts/mem0_delete.py --all --confirm
```
...
```bash
python3 .Codex/skills/memory/scripts/daily_log.py --content "Completed memory system overhaul" --type event
```

## Deprecated

`mem0_sync_md.py` was used to regenerate the legacy `/root/aios/memory/MEMORY.md` from Pinecone. Since the May 2026 migration, layer 1 is Codex's native auto-memory (Codex-managed). Running this script would clobber Codex's writes. The script is kept on disk only for one-shot manual exports if explicitly needed.

```bash
# DO NOT RUN under normal circumstances
# python3 .Codex/skills/memory/scripts/mem0_sync_md.py
```
...
- mem0 config: `.Codex/skills/memory/references/mem0_config.yaml`
...
- `.gitignore` excludes `.env`, `data/`, `memory/logs/`, `.Codex/settings.local.json`.
